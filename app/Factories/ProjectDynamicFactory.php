...
class ProjectDynamicFactory
{
    ...
    public static function make(array $types, array $row): array
    {
        return [
            ...
            'is_network' => self::StringToBoolean($row[3] ?? null),
            'worker_count' => $row[4] ?? null,
            'has_outsource' => self::StringToBoolean($row[5] ?? null),
            'has_investors' => self::StringToBoolean($row[6] ?? null),
            'deadline' => isset($row[7]) ? self::ExcelDateToDate($row[7]) : null,
            'is_on_time' => self::StringToBoolean($row[8] ?? null),
